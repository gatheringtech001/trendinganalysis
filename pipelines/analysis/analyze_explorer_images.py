...
def load_pending(
    db_path: Path, store_id: str, category_group: str, position: int,
) -> PendingAnalysis:
    ...
    rows = connection.execute("""
        ...
        WHERE p.store_id = ? AND p.category_group = ? AND i.position = ?
          AND a.product_id IS NULL
        ORDER BY p.product_id
    """, (store_id, category_group, position)).fetchall()
    connection.close()
    ...
    for index, same_image in enumerate(rows_by_url.values(), 1):
        row = same_image[0]
        key = f"{store_id}:{category_group}:{position}:{index}"
...
def _analyzer(args, usage_callback) -> AzureOpenAIFashionAnalyzer:
    options = AzureOpenAIOptions(
        ...
        deployment=args.deployment, auth_type="api_key",
    )
...
def run_store(args, store_id: str) -> dict:
    ...
    stem = f"image_analysis_{args.category_group.lower()}_cover_{store_id}"
...
def parse_args():
    ...
    parser.add_argument("--category-group", default="SKIRTS")
    ...
    if args.deployment != "gpt-5.6-terra":
        parser.error("usage pricing is configured only for gpt-5.6-terra")
